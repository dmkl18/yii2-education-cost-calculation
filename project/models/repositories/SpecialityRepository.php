...
/*
    Класс, предназначенный для взаимодействия с таблицей БД 'speciality'
        Таблица содержит непосредственно специальности
...
class SpecialityRepository extends BaseRepository
{
    ...
    protected static $tableName = 'speciality';
	...
	public function __construct() 
	{
		$this->dbAttributes = [
			'{{speciality}}.[[id]] as [[id]]',
			'{{speciality}}.[[name]] as [[name]]',
		];
		parent::__construct();
	}

    public function getOne($id) {
        return (new \yii\db\Query())->from('speciality')->where(['id' => $id])->one();
    }

    public function getAll() {
         return (new \yii\db\Query())->from('speciality')->all();
    }
